...
3. **Replaced invalid sentinel values with `NaN`.** Separately from the `tempo` values already stored as `NaN` (22,114 of them), a few columns used a literal `0` to stand in for an unmeasurable value: **1 track** had a `duration_ms` of 0, **89 tracks** had a `tempo` of 0, and **163 tracks** had a `time_signature` of 0. A duration, tempo, or time signature of exactly 0 is physically impossible for a real recording, so these are failed measurements rather than true values. Because they were stored as literal zeros rather than `NaN`, they do **not** show up in an `isna()` check — so I explicitly converted them to `NaN` so they'd be treated as missing rather than dragging down averages. (The 89 zero-tempos were folded in with the 22,114 already-missing tempos.)
